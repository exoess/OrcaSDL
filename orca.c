...
void
orcaselect(int x, int y, int w, int h)
{
	Rect2d r;
	r.x = clmp(x, 0, HOR - 1);
	r.y = clmp(y, 0, VER - 1);
...
{
	orcaselect(cursor.x, cursor.y, cursor.w + (w * (skip ? 4 : 1)), cursor.h + (h * (skip ? 4 : 1)));
}

void
move(int x, int y, int skip)
{
	orcaselect(cursor.x + (x * (skip ? 4 : 1)), cursor.y + (y * (skip ? 4 : 1)), cursor.w, cursor.h);
}
...
{
	switch(option) {
	case 3: orcaselect(cursor.x, cursor.y, 1, 1); break;
	case 8:
		PAUSE = 1;
		frame();
		break;
...
dokey(SDL_Event *event)
{
	...
	if(ctrl) {
		...
		case SDLK_a: orcaselect(0, 0, doc.grid.w, doc.grid.h); break;
